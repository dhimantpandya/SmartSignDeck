  ...
  const handleOpenCloudinaryWidget = async () => {
    ...
    try {
      ...
      window.cloudinary.openUploadWidget({
        ...
        apiKey: api_key,
        // 🔑 Dynamic Signature: Fetch a fresh signature for the exact params generated by the widget
        // This handles additional params like 'custom_coordinates' or 'source=uw'
        uploadSignature: async (callback: any, paramsToSign: any) => {
          try {
            const { signature } = await apiService.get<any>('/v1/cloudinary/signature', {
              params: paramsToSign
            })
            callback(signature)
          } catch (err) {
            console.error('[CLOUDINARY] Failed to generate dynamic signature:', err)
          }
        },
        multiple: false,
        cropping: true,

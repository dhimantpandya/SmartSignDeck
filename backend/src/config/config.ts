...
const config = {
  ...
  email: {
    ...
    gmailClientId: envVars.GMAIL_CLIENT_ID?.trim().replace(/^["']|["']$/g, ""),
    gmailClientSecret: envVars.GMAIL_CLIENT_SECRET?.trim().replace(/^["']|["']$/g, ""),
    gmailRefreshToken: envVars.GMAIL_REFRESH_TOKEN?.trim().replace(/^["']|["']$/g, ""),
  },
  apiDoc: {

...
async function hasYarn(): Promise<boolean> {
  try {
...
// Define constants
const ownPath: string = process.cwd();
const folderName: string = process.argv[2];
const appPath: string = path.join(ownPath, folderName);
const repo: string = 'https://github.com/chintanshahts/node-express-mongo-boilerplate.git';
...
async function setup() {
  try {
    ...
    const useYarn: boolean = await hasYarn();
